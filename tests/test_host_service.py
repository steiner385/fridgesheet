...
def test_linux_remove_disables_and_deletes(tmp_path):
    ...
    def absent(argv, **kw):                                    # a unit that was never installed
        return subprocess.CompletedProcess(argv, 1, stdout="", stderr="Failed to disable unit: Unit file lakota-web.service does not exist.")
...
def test_linux_remove_raises_when_disable_really_fails(tmp_path):
    ...
    assert (tmp_path / "lakota-web.service").exists()           # nothing was deleted either
...
def test_linux_install_reports_a_systemctl_failure(tmp_path):
    ...
    def failing_enable(argv, **kw):
        calls.append(argv)
        if argv[-3:] == ["enable", "--now", "lakota-web.service"]:
...
def test_windows_describe_reads_status():
    ...
    assert calls == [["schtasks", "/Query", "/TN", "Lakota Sheet - web", "/FO", "LIST", "/V"]]
    ...
    assert calls2 == [["schtasks", "/Query", "/TN", "Lakota Sheet - web", "/FO", "LIST", "/V"]]
