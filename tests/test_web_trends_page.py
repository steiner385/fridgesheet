...
def test_the_content_column_can_shrink_below_its_content():
    ...
    assert len(tracks) == 3, "expected the wide layout, the narrow (max-width: 1023px) override and the sideways-phone one"
    assert [t.strip() for t in tracks] == ["var(--rail) minmax(0, 1fr)", "minmax(0, 1fr)", "minmax(0, 1fr) auto"]
