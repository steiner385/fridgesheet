...
- **A HAC grade beats Canvas's automatic "missing"**, whichever source the family prefers.
  ...
  *after* HAC's grade, in a later refresh; then the app asks instead of deciding. Example:
  ...
  sheet reads the same database rows, so it asks too; only a sheet built from a bare
  snapshot (a household that has never recorded a refresh) has no history and follows HAC's
  grade here.
...
| **Open work page** | The sheet's two questions on a screen, per kid: *Still fixable* is the actionable rows (below) no more than the Overdue days setting past due, soonest-closing late-work window first, each with the day it closes and the credit the register promises; *Coming due* is unsubmitted Canvas work due within the Days ahead setting that nothing has happened to yet (*not due yet*). What is open but past its window, and what you flagged handled, are counted underneath with links, not listed. The printed sheet is built from these same rows (`reports/open_work.py`, #137), so the page lists exactly what the sheet prints. Each row is the work list's row: the item's verdict, its question tag, and its detail card. |
...
| **The printed sheet** | The Open work page's rows, on paper: *Still fixable* then *Coming due*, each in the sheet's word for its status -- MISSING, ZERO, LATE, PAPER — CHECK, IN CLASS — CHECK, HAC — NO GRADE, and DUE TODAY / DUE TONIGHT / DUE TOMORROW / DUE *day* (`sheet.STATUS_WORD`, the one table `status_word` reads back for a kid's tier) -- and the page's *Not shown* counts as its two trailers. The runner hands the report the database, and the sheet reads the same decided items the page shows whenever the database holds the snapshot being printed (`reports.open_work.decided_work`); a household that has only ever run a bare `refresh` gets `open_items` over the snapshot, which follows the same rules (`tests/test_open_work_parity.py` holds both). So: a HAC-only row prints HAC — NO GRADE the moment it is past due; undated work prints once the teacher has marked it missing or scored it zero, with no due date and no window (it can never fall out of one); a pair the title matcher misses is paired by date and points on paper as in the app (`matching.twin_by_date_and_points`); in-class work with no grade is IN CLASS — CHECK, not MISSING (#137). A deadline in the first hour of a day (00:00–00:59) belongs to the evening before: it is DUE TONIGHT on that day, never "due tomorrow" the evening it has to be finished, and the app's "Due tonight" and "due today" count say the same (`dates.deadline_date`). Work marked EXC in HAC does not print. Work with a grade above zero in HAC is *done on paper* and does not print, even where Canvas still shows MISSING or a placeholder zero (unless the database saw Canvas mark it after the grade, above); a 0 in either source prints, as ZERO. The sheet keeps its own order -- overdue first, by due date -- where the page sorts *Still fixable* by the window that closes soonest. |
...
from Canvas. Both steps are one matcher (`matching.pair_titles`, `matching.twin_by_date_and_points`)
for the database and the sheet; the sheet paired by title alone until #137.
