...
| **Open work page** | The printed sheet on a screen, per kid: *Still fixable* is the actionable rows (below), soonest-closing late-work window first, each with the day it closes and the credit the register promises; *Coming due* is unsubmitted Canvas work due within the Days ahead setting. What is open but past its window, and what you flagged handled, are counted underneath with links, not listed. |
