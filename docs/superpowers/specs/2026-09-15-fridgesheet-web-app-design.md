# Fridge Sheet as a local web app: design
...
| Today | Becomes |
|---|---|
| tkinter window (`fridgesheet/app/gui.py`) | Removed. The browser is the only UI. |
...
One Python process, `fridgesheet web`, runs a FastAPI application on uvicorn and
...
`<home>/fridgesheet.db` (SQLite through the standard library). Settings stay in
...
fridgesheet/
  ...
  host/
    scheduling_linux.py   gains install/remove that write ~/.config/systemd/user/fridgesheet-<key>.{service,timer}
...
reused unchanged except where named below. `fridgesheet/app/` (actions, gui,
`__main__`) is deleted; the PyInstaller entry point becomes `fridgesheet/web/__main__.py`
...
that writes `~/.config/systemd/user/fridgesheet-<key>.service` and `.timer`
...
account with no desktop session, no request is ever loopback, so "open Fridge Sheet on the
...
Scheduled runs are the OS timers calling the CLI (`fridgesheet run <key>`), as
...
|---|---|---|
| Linux | `fridgesheet-web.service` user unit, `Restart=on-failure`, installed by `fridgesheet service install` | systemd user timers, written by the app for new schedules; existing ones untouched |
| Windows | logon task "Fridge Sheet - web" running `FridgeSheet.exe web --no-browser`, registered by the installer; Start menu shortcut opens the browser | Task Scheduler tasks, one per schedule |
| macOS | launchd agent: adapter shape provided, packaging out of scope | launchd, same |

`FridgeSheet.exe` with no arguments: if the port answers, open the browser; else start
the server in the background and open the browser once it answers. `fridgesheet web`
...
- The PyInstaller spec adds `web/templates` and `web/static` as data; the smoke test
  gains "start `FridgeSheet.exe web --no-browser` on a free port, fetch `/`, fetch
...
- The installer registers the logon task and the uninstaller removes it along with the
  scheduled tasks; the "your data was kept" message now names `fridgesheet.db`.
...
- Deleted: `fridgesheet/app/`, `tests/test_app_*.py`, the `app` CLI command. The
...
- **B. Server.** FastAPI app, Dashboard, Kid, Reconcile, Settings, Diagnostics, Runs;
  jobs with live progress; `host/service.py`; `fridgesheet web` and `service`
