...
| `schedule_fires` table (schema 8, new) | `key TEXT PRIMARY KEY, slot TEXT NOT NULL` — the last slot fired, per schedule key, as an ISO datetime with offset. | `web/db.py` migration |
