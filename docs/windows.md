...
Fridge Sheet's own background server (the one the installer set to start when you sign in) does the work itself — there is no separate Windows task per schedule — while you are **logged in** (a locked screen is fine; a signed-out or powered-off PC skips that run, and a report run caught up the next day does not print the old sheet). If the server was only briefly unavailable, it catches up the moment it is back, as long as that is still the same day and at or after the time you chose. It runs two kinds of schedule:
