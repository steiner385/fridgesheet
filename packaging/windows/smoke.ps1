...
# Checks on the built exe, all against a throwaway LAKOTA_GRADES_HOME so nothing
...
if ($printer) { $env:LAKOTA_PRINTER = $printer; Write-Host "  using printer: $printer" }
...
try {
    ...
    $q = & schtasks /Query /TN "Lakota Sheet - open-work" 2>&1
    ...
    #     LAKOTA_WEB_PORT -- and `service remove`'s /End stops it again.
    ...
    Remove-Item Env:\LAKOTA_GRADES_HOME -ErrorAction SilentlyContinue
    Remove-Item Env:\LAKOTA_PRINTER -ErrorAction SilentlyContinue
    Remove-Item Env:\LAKOTA_WEB_PORT -ErrorAction SilentlyContinue
    Remove-Item Env:\LAKOTA_WEB_NO_BROWSER -ErrorAction SilentlyContinue
}
