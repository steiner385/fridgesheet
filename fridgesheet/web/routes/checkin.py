...
def _context(conn, student, state):
    ...
    views = items.list_items(conn, student, now=now, rules=rules, show="all", prefs=state.sources(), **state.window())
...
def _form_context(conn, student, state, item_id=None, step_id=None, default_state="planned"):
    ...
    view = items.one(conn, student, item_id, now=state.now(), rules=state.rules(), prefs=state.sources(), **state.window()) if item_id else None
    if values is None:
