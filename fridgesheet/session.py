...
log = logging.getLogger("fridgesheet.session")
...
def onelogin_login(page: Page, settings: Settings) -> None:
    ...
    if user_field is None:
        raise LoginRequired(
            "Could not find the OneLogin username field (override FRIDGESHEET_ONELOGIN_USER_SELECTOR). "
            f"At {page.url}"
        )
    ...
    if pw_field is None:
        raise LoginRequired(
            "Could not find the OneLogin password field (override FRIDGESHEET_ONELOGIN_PASS_SELECTOR). "
...
def onelogin_app_url(page: Page, settings: Settings, pattern: str) -> str | None:
    ...
    located by name rather than pinned. Set FRIDGESHEET_HAC_ONELOGIN_APP_URL to skip this.
    """
        ...
        if "/Account/LogOn" in page.url:
            ...
            if not app_url:
                raise LoginRequired(
                    ...
                    "Set FRIDGESHEET_HAC_ONELOGIN_APP_URL to the tile's launch URL, or adjust "
                    f"FRIDGESHEET_HAC_APP_PATTERN (currently {settings.hac_app_pattern!r})."
                )
