...
def _win32print():
    try:
        ...
        raise PrintError("pywin32 is not installed; reinstall Fridge Sheet or `pip install fridgesheet[windows]`") from e
    return win32print
...
def print_pdf(pdf: Path, printer: str | None, title: str, run=subprocess.run, now: datetime | None = None) -> str:
    exe = sumatra_path()
    if not exe.is_file():
        raise PrintError(f"SumatraPDF not found at {exe}; reinstall Fridge Sheet")
