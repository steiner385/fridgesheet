"""Command line: `lakota-grades login|set-credentials|check|refresh|status|serve|run|reports|printers|schedule|print-sheet|web|service|doctor`."""
...
def _removal_settings() -> tuple[Settings, str]:
    ...
    - It ends with `for d in (s.home, s.profile_dir, s.cache_dir): d.mkdir(...)`. That defeats
      ...
      `db_path.is_file()`: a parent who deleted `%LOCALAPPDATA%\\lakota-grades` before
...
def _schedule_removal_keys(s) -> tuple[set[str], bool]:
    ...
    - The database's own saved-report listing, `[reports."view:<id>"]`'s other half -- but
      *only* read when `db.db_path(s.home)` already exists. `reports.available(home)` calls
      ...
      `%LOCALAPPDATA%\\lakota-grades` before uninstalling a freshly recreated one back, while
    ...
    build does not understand is the concrete case: install a build that migrates `lakota.db` to
    ...
    "Lakota Sheet - Web", which in Task Scheduler's case-insensitive namespace *is* the web
...
def main(argv=None) -> None:
    ...
    ps.add_argument("--printer", default=os.environ.get("LAKOTA_PRINTER") or None, help="printer name (default: config.toml, then the system default)")
