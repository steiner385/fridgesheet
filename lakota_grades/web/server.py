...
browser there and exits 0 -- that is what the desktop shortcut and `LakotaSheet.exe` with no
arguments do.
"""
...
def run(settings: Settings, *, host: str | None = None, port: int | None = None, open_browser: bool = True,
    ...
    if host is not None or port is not None:
        ...
        # LAKOTA_WEB_HOST/LAKOTA_WEB_PORT already do it via `web_host_explicit` -- so the
