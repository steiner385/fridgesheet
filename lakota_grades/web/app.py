...
def served_url(settings: Settings) -> str:
    ...
    `LAKOTA_WEB_HOST=192.168.1.42` uvicorn binds *that instead of* loopback, so advertising
...
def _allowed_hosts(settings: Settings) -> set[str]:
    ...
    for `allow_lan` *and* an explicit `LAKOTA_WEB_HOST` pin winning over it (see
    `Settings.bind_host`). A concrete bind (including a `LAKOTA_WEB_HOST` pin to one specific
...
def create_app(settings: Settings, *, home: Path | None = None, worker: bool = False) -> FastAPI:
    ...
    async def same_origin_only(request: Request, call_next):
        ...
        if not _host_allowed(host, settings):
            ...
            return HTMLResponse(
                f"<p>Lakota Sheet only answers at <a href=\"{url}\">{url}</a> on this computer.</p>"
                "<p>Open that address on the computer running Lakota Sheet. To read it on a phone "
                "or tablet, tick <b>Allow other devices on this network</b> on the Settings page, "
                "restart Lakota Sheet, and use the address (or the QR code) shown there.</p>",
                status_code=403)
