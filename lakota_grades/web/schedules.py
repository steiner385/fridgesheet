...
log = logging.getLogger("lakota.web.schedules")
...
def rows(home: Path, *, scheduling=None) -> list[Row]:
    ...
    for report in registry.available(home):
        ...
        try:
            ...
            # Nothing under `lakota_grades/host/` raises this for scheduling today -- both
...
def _unmanageable(key: str, title: str, scheduling) -> str | None:
    ...
    straight through the `not enabled` branch: the hand-written `lakota-print-sheet.timer`
