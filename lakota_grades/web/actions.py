...
def save(form: FormValues, *, home: Path, log: Callable[[str], None], credstore=None) -> SaveResult:
    ...
    if restart_needed:
        msg = "The server address changed; restart Lakota Sheet (or the service) for it to take effect."
        log(msg)
        messages.append(msg)
...
def about_text() -> str:
    ...
    return (
        ...
        "asks GitHub whether a newer Lakota Sheet exists (nothing is sent; turn it off on this page)."
    )
