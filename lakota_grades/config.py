...
class Settings:
    ...
    web_host_explicit: bool = False    # LAKOTA_WEB_HOST was set: that address wins over allow_lan
    ...
    def bind_host(self) -> str:
        """The address to bind. An explicit LAKOTA_WEB_HOST wins over `allow_lan`, so
        `LAKOTA_WEB_HOST=127.0.0.1` pins the server to loopback whatever config.toml says."""
    ...
    def credentials(self) -> tuple[str, str]:
        ...
        if not (user and pw):
            ...
            # by telling them to run a command they do not have -- `lakota-grades` is not on
            # PATH for anyone who installed the Windows app, where the executable is
            # LakotaSheet.exe. The CLI and environment routes still matter, and still follow,
            ...
            raise RuntimeError(
                ...
                "(From a terminal instead: `lakota-grades set-credentials` to store them in the "
                "OS credential store, or LAKOTA_ONELOGIN_USERNAME/PASSWORD in the environment, "
                "or LAKOTA_OP_USERNAME_REF/LAKOTA_OP_PASSWORD_REF for the 1Password CLI.)"
            )
...
def load_settings() -> Settings:
    ...
    env_host = os.environ.get("LAKOTA_WEB_HOST")
    if env_host:
