...
The sheet on the fridge: a parent's daily list of open school work, pulled from **Canvas** (assignments, due dates, missing/late flags) and **Home Access Center** (official grades) for every kid in the house, decided once by [one definition of done](docs/outcomes.md), and printed or shown on a browser app. It started for one district (the Windows build is still called "Lakota Sheet") and works for any district that runs those two systems behind OneLogin. It also serves the same data to Claude as a local MCP server, so weekly check-in reports can be built without driving Chrome.

The Python package and the Linux install path still carry the old name, `lakota_grades` / `~/lakota-grades-mcp`; those rename in a later release with a migration for existing installs.
...
- **Other parents can install it.** A Windows build ("Lakota Sheet") with a browser app and a per-user installer: see [docs/windows.md](docs/windows.md) and "Releasing" below.

## 1. Install (Linux)

```bash
git clone https://github.com/steiner385/fridgesheet ~/lakota-grades-mcp
...
```json
{
  "mcpServers": {
    "lakota-grades": {
      "command": "/home/<you>/lakota-grades-mcp/.venv/bin/lakota-grades",
      "args": ["serve"],
      "env": { "LAKOTA_ENV_FILE": "/home/<you>/.lakota-grades/.env" }
    }
  }
}
```
...
| `print-sheet.log` | One line per run with its outcome, preceded by an `INFO ingested …` line on runs that refreshed. The same lines go to stderr, so `journalctl --user -u lakota-print-sheet` has them too. |
| `lakota.db` | The app's database: every refresh as a change log, plus notes, flags and run history. SQLite in WAL mode, so `lakota.db-wal` and `lakota.db-shm` sit beside it; copy all three or none. Deleting it costs the history, not the sheets. |
...
`lakota-grades run open-work` is the general form of `print-sheet` (same flags); `print-sheet` always passes its own `--days`/`--overdue-days` defaults (14), so `days_ahead`/`overdue_days` in `config.toml` apply to `run open-work` but not to the alias. `lakota-grades reports` lists report types and their schedules; `lakota-grades schedule show <key>` prints the next run and who manages it. `lakota-grades schedule install <key>` writes the schedule itself — the Task Scheduler task on Windows, a systemd user timer pair on Linux (below).

**Upgrading from 0.1:** the built-in `Alex=Al` nickname is gone. Add `LAKOTA_NICKNAMES=Alex=Al` to `.env` or the `[kids]` table above, or the sheet prints the full first name. The built-in `Brother_MFC_J4335DW` printer default is also gone; the systemd unit still sets `LAKOTA_PRINTER`, but the desktop shortcuts run without it, so add `LAKOTA_PRINTER=<your CUPS printer name>` to `~/.lakota-grades/.env` (or `[print] printer` in `config.toml`) or those shortcuts print to the CUPS default.
...
2. Optionally, after step 1 has merged (GitHub only offers the Run workflow button for workflows on the default branch), run the workflow by hand first (Actions → release → Run workflow) and download the `LakotaSheet-Setup` artifact to try it.
3. Tag and push: `git tag v0.3.1 && git push origin v0.3.1`. The job refuses a tag that does not match `pyproject.toml`, and on success attaches `LakotaSheet-Setup-0.3.1.exe` to a GitHub release. Installed copies notice the new release within a day (Settings shows *Lakota Sheet 0.3.1 is available* with a download link; the header carries a badge) and the installer upgrades over the running app.
