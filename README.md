...
This is unrelated to report scheduling below — it only keeps the snapshot warm, and it is not something the app will ever touch: the app's own schedules live only in `config.toml`, never in a systemd unit, so a hand-written pair like this one is safe by construction, not by name-based refusal. A report's own printing schedule, including the weekday sheet's, is what the server's own clock fires itself, once a minute, while it is running (below).

The app can now schedule this itself, on Windows as well as Linux: tick **Refresh on a schedule** on the Schedules page and set an interval and a window. It writes only `[refresh]` in `config.toml` — no systemd unit, no Task Scheduler task — so a hand-written timer from this section and the app's own schedule can both exist on one machine; the app never touches the hand-written pair. If you use the app's schedule, disable the hand-written one yourself:

    systemctl --user disable --now fridgesheet-refresh.timer

## 6. Printing a report on a schedule

`fridgesheet run <key>` refreshes, builds one report's PDF, prints it, and records the run. A schedule turned on for that report does exactly this, in-process, with `--no-refresh` built in (below): the same "one pull, many tools" promise as section 4's tools, so a scheduled print never waits on, or repeats, a Canvas/HAC pull the refresh schedule above already did. Typed by hand with no flag, `run` and `print-sheet` still refresh first, same as always — someone at a terminal is presumably fine waiting a few minutes; so does the household's hand-written print timer, which the app never touches. A report key is `open-work` (the built-in kids' sheet: one letter-portrait PDF with a section per kid) or `view:<id>` (a report built on the Reports page). `fridgesheet reports` lists every key with its enabled state, schedule and whether it's PDF-only:
...
`fridgesheet print-sheet` still exists — it's `run open-work` under its original name, sending the PDF to CUPS as one duplex job, except it always applies its own `--days`/`--overdue-days` defaults (14) rather than `config.toml`'s. The household's live hand-written timer still calls it by that name (and still refreshes); a schedule turned on since, on the Schedules page or by hand in `config.toml`, is what the server's clock runs as `run <key> --no-refresh` instead.
...
```toml
[general]
timezone = "America/Chicago"          # IANA name; leave it out for this computer's own zone.
                                      # Due times, "today", the print window and schedules follow it.
...
`fridgesheet run open-work` is the general form of `print-sheet` (same flags); `print-sheet` always passes its own `--days`/`--overdue-days` defaults (14), so `days_ahead`/`overdue_days` in `config.toml` apply to `run open-work` but not to the alias. `fridgesheet reports` lists report types and their schedules; `fridgesheet schedule show <key>` prints its next run and how its last scheduled run went. Turning a schedule on writes `[reports.<key>]` in `config.toml` itself — from the Schedules page, or by hand (below) — there is no separate install step. `fridgesheet schedule install <key>` is the same switch from a terminal (`install data-refresh` for `[refresh]`), and `remove` turns one off; neither touches Task Scheduler or systemd. A `config.toml` that does not parse, or a setting that does not read, is one line naming the file and the setting and exit code 2 from every command; the Settings and Schedules pages show the same message and withhold their forms rather than save over it.
...
```bash
fridgesheet schedule show open-work       # next run, and how its last scheduled run went
fridgesheet schedule install open-work    # turns it on in config.toml (data-refresh: the [refresh] schedule)
fridgesheet schedule remove open-work     # turns it off in config.toml
fridgesheet schedule remove --all         # remove tasks/timers an earlier version left with the OS -- what the Windows uninstaller runs
```

`schedule install` creates no OS task any more. Turning a schedule on, from here or the Schedules page, writes only `[reports.<key>]` in `config.toml` — days, time, printer, whether it prints or just builds the PDF — no systemd unit, no Task Scheduler task; the printer is read at run time, per the precedence above. The server's own clock (`fridgesheet/web/clock.py`) reads `config.toml` once a minute and fires whatever is due, but only while the server itself is running: `fridgesheet service install` on Linux, the installer's own logon task on Windows. A slot missed while the server was down is caught up once when it comes back; a print caught up the next morning is refused by the same print-window guard described below. Because the app never writes to a systemd unit or Task Scheduler task for a report's own schedule, it can never clobber `fridgesheet-print-sheet.{service,timer}` above, or any other hand-written unit — there is nothing to refuse, there is simply nothing written. `schedule remove --all` is the one command that still talks to the OS: it removes whatever a pre-in-app-scheduler version of Fridge Sheet registered, so upgrading from a release that used to write those leaves nothing behind.
...
| `upcoming(student, days=14)` | Unsubmitted items due in the window, in the configured time zone, with DUE TODAY / DUE TOMORROW / DUE <weekday> statuses |
...
All dates are ISO strings in the configured time zone — `[general] timezone`, or this computer's own (Canvas `due_at` is UTC and is converted).
